...
def test_workbook_batch_inspection_combines_independent_sheet_results(tmp_path: Path):
    ...
    first = wb.active
    first.title = "Set A"
    first.append(["sample", "value"])
    first.append(["A1", 1])
    first.append(["A2", 2])

    second = wb.create_sheet("Set B")
    second["A1"] = "time"
    second.merge_cells("B1:C1")
    second["B1"] = "Group"
    second.append(["t0", 1, 2])
    second.append(["t1", 3, 4])
    # Empty formatting residue must not become meaningful geometry.
    second.merge_cells("A3000:B3000")
    second["J5000"].fill = PatternFill(fill_type="solid", fgColor="FFFFFF")

    third = wb.create_sheet("Set C")
    third.append(["x", "y", None, None, "u", "v"])
    third.append([1, 2, None, None, 3, 4])
    third.append([5, 6, None, None, 7, 8])
    wb.save(path)
