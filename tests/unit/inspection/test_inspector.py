...
def test_xlsx_multiple_regions_require_explicit_selection(tmp_path: Path):
    ...
    for row in (("a", "b", None, "x", "y"), (1, 2, None, 3, 4), (5, 6, None, 7, 8)): ws.append(row)
    wb.save(path)
    ...
    assert [r.a1_range for r in report.regions] == ["A1:B3", "D1:E3"]
...
def test_selected_region_limits_inspection_and_detects_merged_header(tmp_path: Path):
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = "Data"
    ws.merge_cells("A1:C1"); ws["A1"] = "Group"
...
def test_inspection_does_not_modify_source_bytes(tmp_path: Path):
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.append(["a", "b"]); ws.append([1, 2]); wb.save(path)
    before = path.read_bytes(); inspect_table(path, sheet_name=0)
...
def test_style_only_extent_does_not_trigger_false_region_scan_truncation(tmp_path: Path):
    ...
    wb = openpyxl.Workbook(); ws = wb.active; ws.title = "Data"
    ws["A1"], ws["B1"] = "sample", "value"; ws["A2"], ws["B2"] = "S1", 1; ws["A3"], ws["B3"] = "S2", 2
    ws.merge_cells("A3000:B3000"); ws["J5000"].fill = PatternFill(fill_type="solid", fgColor="FFFFFF"); wb.save(path)
    report = inspect_table(path, sheet_name="Data")
