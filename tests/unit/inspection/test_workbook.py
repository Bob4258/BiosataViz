...
def _make_batch_workbook(path: Path):
    ...
    clean = wb.active
    clean.title = "Clean"
    clean.append(["sample", "value"])
    clean.append(["S1", 1])
    clean.append(["S2", 2])

    merged = wb.create_sheet("Merged")
    merged["A1"] = "time"
    merged.merge_cells("B1:C1")
    merged["B1"] = "Group"
    merged.append(["t0", 1, 2])
    merged.append(["t1", 3, 4])

    multi = wb.create_sheet("Multi")
    multi.append(["a", "b", None, None, "x", "y"])
    multi.append([1, 2, None, None, 3, 4])
    multi.append([5, 6, None, None, 7, 8])

    last = wb.create_sheet("Last")
    last.append(["sample", "value"])
    last.append(["L1", 5])
    last.append(["L2", 6])
...
def test_inspect_workbook_explicit_subset_preserves_caller_order(tmp_path: Path):
    ...
    path = tmp_path / "batch.xlsx"
    _make_batch_workbook(path)
    report = inspect_workbook(path, sheets=["Last", "Clean"])
    assert report.sheet_names == ("Last", "Clean")
...
def test_inspect_workbook_resolves_integer_indices_and_rejects_duplicates(tmp_path: Path):
    ...
    path = tmp_path / "batch.xlsx"
    _make_batch_workbook(path)
    report = inspect_workbook(path, sheets=[3, 0])
    assert report.sheet_names == ("Last", "Clean")
...
def test_inspect_workbook_rejects_missing_sheet_and_csv(tmp_path: Path):
    ...
    with pytest.raises(ExcelSheetNotFoundError):
        inspect_workbook(path, sheets=["Missing"])

    csv_path = tmp_path / "data.csv"
    csv_path.write_text("a,b\n1,2\n", encoding="utf-8")
    with pytest.raises(UnsupportedTableFormatError):
        inspect_workbook(csv_path)
...
def test_workbook_derived_counts_and_source_immutability(tmp_path: Path):
    ...
    merged = next(item for item in report.sheets if item.sheet_name == "Merged")
    multi = next(item for item in report.sheets if item.sheet_name == "Multi")
